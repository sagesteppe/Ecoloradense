---
title: "Download and Process Independent Variables for the Upper Gunnison"
format: html
---

## Overview

SDM analyses will be performed at 3 spatial resolutions: 90m (3 arc-second),
30m (1 arc-second), and 10m (1/3 arc-second). These resolutions will be
compared via ground truthing, generating an interesting, albeit small,
intermediary paper: how do these different resolution data perform for
predicting presence and absence?

In order for the most possible comparisons of these products, each predictor
is generated from a DEM native to the above resolutions, using the same
functions and processes.

Predictions from the more coarse grain products will be resampled to 3m
resolution for equivalency in ground truthing - i.e. equal probabilities of
detection of individuals.

Every chunk below is set to `eval = FALSE`. This document records a
data-processing pipeline that reads/writes very large rasters (multi-GB to
tens of GB) and shells out to external tools (whitebox, ClimateNA, MrSID
decoder, Copernicus/Sentinel downloads) - it is meant to be stepped through
chunk-by-chunk by hand, never rendered/knit end-to-end.
...
The 'closest' this bounding box is to a known occurrence is 10 miles. The
furthest distances vary.

```{r define domain, eval = FALSE}
...
```{r tile 1m dem redefine, eval = FALSE}
tiles1m <- function(x, domain){
...
Now we will resample these tiles to 3m resolution - USGS does not have a
product at this resolution.

```{r crop 1m tiles to domain, eval = FALSE}
...
```{r aggregate 1m tiles to 3m, eval = FALSE}
...
## Geomorphology from the 1m DEM tiles

```{r morphoMaker function, eval = FALSE}
#' morphoMaker
...
## Cost surfaces between known occurrences

Cost surfaces between known occurrences can be generated via modelling; kept
here as a reference to the relevant whitebox help pages.

```{r cost surface references, eval = FALSE}
...
## NAIP imagery extraction

The NAIP data were processed using bash, and left at their native resolution.
It unfortunately requires that we extract the .sid files from the .zip files.

```{bash extract NAIP sid files, eval = FALSE}
...
We will write these data to generate rasters using ClimateNA for the study
area. We will generate projections at 3arc.

```{r prep DEM for ClimateNA, eval = FALSE}
setwd('/media/steppe/hdd/EriogonumColoradenseTaxonomy/scripts')
...
ClimateNA was run on ~90m resolution data, which is roughly 11% of the native
resolution of PRISM data. We will bring these data into the lower resolutions
so they can be used with each modelling approach.

```{r resample climateNA, eval = FALSE}
...
## NAIP products at each resolution

```{r resample NAIP, eval = FALSE}
setwd('/media/steppe/hdd/EriogonumColoradenseTaxonomy/data/spatial/processed/')
...
### GLCM texture for the NAIP imagery

```{r NAIP GLCM texture, eval = FALSE}
setwd('/media/steppe/hdd/EriogonumColoradenseTaxonomy/data/spatial/processed/')
f <- vrt(file.path('NAIP', list.files('NAIP', pattern = 'tif')))
...
## Vegetation cover at each grain

We want to combine the trees into a single dataset for 'forest'.

```{r resample vegetation cover, eval = FALSE}
...
## Latitude and longitude layers for each resolution

```{r coordinate rasters, eval = FALSE}
setwd('/media/steppe/hdd/EriogonumColoradenseTaxonomy/data/spatial/processed/')
...
## Snow free days product from Sentinel-2 imagery

```{r CDSE auth, eval = FALSE}
library(CDSE)
library(terra)
library(sf)
...
```{r search Sentinel-2 catalog, eval = FALSE}
...
```{r download NDSI tiles, eval = FALSE}
...
### Determine the last date snow was found in each cell each year

```{r NDSI exploration, eval = FALSE}
p <- '/media/steppe/hdd/Geospatial_data/Sentinel2EriogonumColoradense/'
setwd(paste0(p, 'NDSI_raw'))
...
Group by tile number, then year, then date. Calculate NDSI for each cell x
time interval.

```{r tile inventory by year, eval = FALSE}
FILES  <- list.files()
tiles <- data.frame(
  FNAME = FILES,
...
First we will create a cloud mask - it won't work perfectly but should
function OK for our purposes. These are the terms for the
braaten-cohen-yang cloud detection index
(https://custom-scripts.sentinel-hub.com/custom-scripts/sentinel-2/cby_cloud_detection/).

```{r cloud detection, eval = FALSE}
...
We want to calculate this during ~ July 10 - July 22 (DOY 192-204). It will
need to be in our high moisture years: 2017, 2019, 2023 - let's aim for the
data set within those years with the absolute lowest amount of cloud cover.

```{r select NDVI acquisition dates, eval = FALSE}
cr_high <- cr |>
...
```{r download NDVI tiles, eval = FALSE}
...
```{r calculate and resample NDVI, eval = FALSE}
setwd(paste0(p, 'NDVI_raw'))
f <- list.files()
...
We can also calculate SAVI using the same NDVI-date imagery.

```{r calculate and resample SAVI, eval = FALSE}
setwd(paste0(p, 'NDVI_raw'))
f <- list.files()
...
A few more geomorphology variables which weren't made with the first batch.

```{r additional curvature layers, eval = FALSE}
morphoMaker <- function(x, p){
...
## Decompose aspect into Northness and Eastness

```{r decompose aspect, eval = FALSE}
x = c(
...
## Heat load index and diurnal anisotropic heat index

A couple of products from the `spatialEco` package which relate to how much
heat areas on a landscape accumulate. `heat()` needs `spatialEco` installed.
Both `dahi()` and `hli()` call `terra::terrain()` internally, so `x` must be
the elevation DEM (`DEM_filled.tif`), not a precomputed aspect raster -
passing aspect in would derive slope/aspect of the aspect surface itself.

```{r heat load and DAHI, eval = FALSE}
x = c(
  './dem_3arc/geomorphology/DEM_filled.tif',
  './dem_1arc/geomorphology/DEM_filled.tif',
  './dem_1-3arc/geomorphology/DEM_filled.tif'
  )
...
heat <- function(x){
  ...
  names(dahi_r) <- 'DAHI'
  terra::writeRaster(dahi_r, file.path(dirname(x), 'DAHI.tif'), overwrite = TRUE)

  hli <- spatialEco::hli(r)
  names(hli) <- 'HLI'
  terra::writeRaster(hli, file.path(dirname(x), 'HLI.tif'), overwrite = TRUE)
...
## Factor rasters for geomorphon and Pennock landform position

Convert geomorphon and Pennock landform positions into factor rasters.

```{r geomorphon and Pennock factors, eval = FALSE}
