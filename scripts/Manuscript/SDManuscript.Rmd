...
## Field-validated Pr-AUC of the best-performing model against the 2026
## opportunistic ground-truthing points, within 270m of existing occurrences
## (produced by Second_groundtruth/digitize_evaluate_2026_groundtruth.R).
## Only the 3 arc-second surface is evaluated for now - the 1 and 1/3
## arc-second suitability surfaces aren't built on this machine yet; add
## their rows to the same table once they are.
gt2026_270 <- read_csv(file.path('..', '..', 'results', 'tables', '2026-groundtruth-evaluation-270m.csv'),
                        show_col_types = FALSE)

best_model_row <- filter(gt2026_270, resolution == '3 arc-second')
best_model_pr_auc <- if (nrow(best_model_row) == 1) round(best_model_row$pr_auc, 2) else 'XX'
best_model_n <- if (nrow(best_model_row) == 1) best_model_row$n else 'XX'
```

Within 270m of the known occurrences, the 3 arc-second model (n = `r best_model_n`) had a PR-AUC of `r best_model_pr_auc`, indicating they can model the species fundamental niche.
...
## phase HO, 'Publically Available Points'
## phase H1, 'Initial Survey'
## phase H2, 'Field Verified'
...
heldout_field_mock <- tibble(
  ...
  mutate(Set = factor(Set, levels = c('Heldout', 'Field'), labels = c('Held-out', 'Field-validated')),
         ## position_dodge infers a per-Phase y-offset for geom_point (it can tell
         ## Resolution is the discrete axis), but not for geom_line - so the
         ## connecting line was landing on the row's true center instead of
         ## alongside its own pair of points. Offset y by hand instead, using
         ## the same magnitude position_dodge(width = 0.5) gives geom_point.
         y_num = as.numeric(Resolution) + ifelse(Phase == 'H0', -0.125, 0.125))

ggplot(heldout_field_mock, aes(y = y_num, x = PrAUC, color = Phase, group = interaction(Resolution, Phase))) +
  geom_line(linewidth = 0.6) +
  geom_point(aes(shape = Set), size = 3, stroke = 1.1) +
  scale_shape_manual(values = c('Held-out' = 1, 'Field-validated' = 16)) +
  scale_color_phase() +
  scale_x_continuous(limits = c(0, 1)) +
  scale_y_continuous(breaks = seq_along(res_levels), labels = res_levels,
                      limits = c(0.5, length(res_levels) + 0.5)) +
  labs(x = 'Area under the precision-recall curve', y = NULL, shape = NULL, color = 'Phase',
       title = '(mock) H1 - Held-out vs. field-validated Pr-AUC, median model per resolution') +
  theme_manuscript()
```
...
## Occupancy ~ Euclidean distance (E) and the region-orthogonalized cost-distance
## anomaly (R) at the 2026 ground-truth points (produced by
## Second_groundtruth/cost_residual_occupancy.R, Seed1021 realization of
## 1-3arc-Iteration1-PA1:1DO:0). Detection bands follow the Introduction's "Here we"
## list: extension < 270m, sub-population 270-450m, new population >= 450m of a known
## occurrence.
## NOTE: these are per-POINT counts - adjacent detections that belong to the same real
## population will be merged into named populations before this is final; a cluster of
## "new population" points 500m apart is one new population, not several.
bands <- read_csv(file.path('..', '..', 'results', 'tables', '2026-groundtruth-occupancy-bands.csv'),
                   show_col_types = FALSE)
get_band <- function(b) { v <- bands$n_presences[bands$band == b]; if (length(v) == 1) v else 'XX' }
n_extension <- get_band('extension (<270m)')
n_subpop    <- get_band('sub-population (270-450m)')
n_newpop    <- get_band('new population (>=450m)')

occ_lrt <- read_csv(file.path('..', '..', 'results', 'tables', '2026-groundtruth-occupancy-glm-lrt.csv'),
                     show_col_types = FALSE)
get_p <- function(term_lbl) { v <- occ_lrt$p_value[occ_lrt$term == term_lbl]; if (length(v) == 1) signif(v, 2) else 'XX' }
p_E_given_R <- get_p('E (given R)')
p_R_given_E <- get_p('R (given E)')
p_E_alone   <- get_p('E (alone)')
p_R_alone   <- get_p('R (alone)')
```

`r n_subpop` sub-populations (occurrences found between 270-450m) and `r n_newpop` new populations (>=450m) were detected while field sampling, plus `r n_extension` extensions of already-known populations (<270m); these per-point detections will be merged into named populations before final reporting.
A logistic model of occupancy on Euclidean distance (E) and the cost-distance anomaly (R) finds that neither predictor is significant alone (E: p = `r p_E_alone`; R: p = `r p_R_alone`), but each becomes significant once the other is included (E given R: p = `r p_E_given_R`; R given E: p = `r p_R_given_E`), indicating the two distance measures are complementary rather than redundant predictors of occupancy.
...
Savanna Smith is thanked for verifying the placement of an undocumented population.
