...
Accordingly, while many ENMs achieve high performance on hold-out test subsets, biases in the training data may be carried into predictions [@a2022species].
To generate more presence and absence records for training, adaptive-niche-based sampling (ANBS) is increasingly used.
... preferentially visiting cells with high predicted occurrence probability or high prediction uncertainty [@guisan2006using]; because such cells are, almost by definition, close to already-known occurrences, this tests a model's easiest predictions rather than confirming whether the occurrence records it was trained on remain accurate on the ground.
...
'Distance' may be defined as Euclidean (or Haversine), or as a least-cost distance reflecting a generalised surface which conveys the difficulty for seeds to travel between the nearest occupied sites and the site of interest [@etherington2016least].
...
Here we: 1) evaluate the results of the best-performing model with groundtruthing (within 270m of existing occurrences).
2) Compare the results of models from different resolutions to the best model, as based on hold-out test data.
3) Determine whether model predictions are useful to find new sub-populations (270m - 540m from existing occurrences), or populations (> 540m from existing occurrences).
...
Its known range spans five counties and encompasses 22 documented populations, occurring across a broad range of elevation (2700-3900 asl), slope, aspects, soil types, and habitats - from high-elevation sagebrush steppe, subalpine grasslands, and alpine slopes.
...
These data were manually reviewed and 16 herbarium records removed due to low geolocation quality or a georeferenced locality inconsistent with the specimen label.

Digital elevation models (DEMs) at 3 arc (type) and 1 arc (type) resolution, and digital elevation products (DEPs) at 1/3 arc resolution, were acquired from the United States Geological Survey and clipped to the analytical domain — a rectangle buffered 16 km (10 mi) beyond any known population [CITE].
...
Collectively these resulted in 50 indepedent variables for ENM modelling. 
...
Subjectively placed absences were also collected in areas that seemed favourable or were in proximity to *E. coloradense* individuals; this occurred both in the field (n = `r opportunistic_absences`).
...
Population boundaries were delineated by thresholding the continuous habitat suitability surface from the best-performing model using two different threshold-selection rules 1) sensitivity, and 2) equal sensitivity and specificity.
*— the threshold maximising the sum of sensitivity and specificity ('spec_sens'), and the threshold at which sensitivity alone is maximised ('sensitivity') — producing two independent candidate delineations of suitable habitat, and polygonized each into discrete patches within ground-truthable areas, retaining only patches ≥ 2 pixels of the source raster's resolution [@hijman2024dismo].*
...
ggplot(heldout_field_mock, aes(y = Resolution, x = PrAUC, color = Phase, group = interaction(Resolution, Phase))) +
  ...
  labs(x = 'Pr-AUC', y = NULL, shape = NULL, color = 'Phase',
...
```{r Plant Density Held-out vs Field-validated MAE Placeholder, out.width='100%'}
## PLACEHOLDER: mock dumbbell plot (synthetic data) previewing the layout
## for held-out vs. field-validated MAE per model, faceted by resolution.
## Swap for the real chunk once field-validated MAE is computed and the
## PA-ratio comparison is collapsed out of the count-model tables.
img <- file.path('figures', 'fig3_density_mae_MOCK.png')
