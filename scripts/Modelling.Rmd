...
gbs <- function(rec, cv, train, test, tune_gr, mode, engine, objective, levels){
...
xgb_tweedie_gr <- lgbm_poisson_model |>
  tune::extract_parameter_set_dials() |>
...
best_param_tweedie <- tune::select_best(params_xg_poisson, metric = "mae")

lgbm_poisson_model <- lgbm_poisson_model |>
  tune::finalize_model(best_param_tweedie)

lgbm_poisson_fit <- lgbm_poisson_model %>% 
  fit(Prsnc_All ~ ., data = train)

preds <- data.frame(
  'Observed' = test$Prsnc_All, 
  'Predicted' = stats::predict(lgbm_poisson_fit, new_data = test),
