...
tune_gr <- parsnip::boost_tree( 
  mode = 'regression',
  trees = c(1, 400, 800, 1200),
  tree_depth = seq(2,8, by = 2),
  min_n = c(1, 3, 7, 10), 
  loss_reduction = tune(),
  learn_rate = tune(), 
  stop_iter = tune()
...
best_xg_nb <- tune::select_best(params_xg_nb, metric = "mae")

xg_nb <- xgb_nb |>
  tune::finalize_model(best_xg_nb)

xgb_nb_fit <- xg_nb %>% 
  fit(Prsnc_All ~ ., data = train)

xbg_nb_pred <- data.frame(
    'Observed' = test$Prsnc_All, 
    'Predicted' = stats::predict(xgb_nb_fit, new_data = test),
    'Pr.suit' = test$Pr.SuitHab
)

Metrics::mae(xbg_nb_pred$Observed, xbg_nb_pred$.pred)
```
...
Metrics::mae(xbg_tweedie_pred$Observed, xbg_tweedie_pred$.pred)
```
...
Metrics::rmse(xbg_tweedie_pred$Observed, xbg_tweedie_pred$.pred)
