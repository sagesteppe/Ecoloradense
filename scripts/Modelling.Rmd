...
# we will create a column which is meant to indicate the spatial configuration of
# our points. Basically, we have a problem which makes splitting along the outcome
# variable, not an ideal choice. If you remember, the southern Cocheotopa Dome (CD) population
# has drastically higher counts of plants than the populations near CB, 10x individuals
...
twinning_dat <- df |>
  mutate( # this algo probably rescales too, but we'll just feed em in to be sure. 
...
indices_knndm <- spatialsample::spatial_nndm_cv(
  train.sf,
  st_as_sfc(st_bbox(rast_dat))
)


rm(ct, indx, twinning_dat)
```
...
train <- select(train, all_of(c('Prsnc_All', predictors(rfProfile))))
```
...
params_xg_poisson <- xgb_poisson |>
      tune::tune_grid(
        rec,
        resamples = indices_knndm$indx_train,
...
lightgbm_model <- parsnip::boost_tree(
    ...
    set_engine("lightgbm", objective = "poisson", verbose=-1)
...
lgbm_wf <- workflows::workflow() %>%
    ...
    workflows::add_formula(Prsnc_All ~ .)
