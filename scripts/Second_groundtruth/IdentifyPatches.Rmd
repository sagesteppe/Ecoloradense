...
To reduce the computational expense we will apply a couple approaches which will make this a somewhat cheapened heuristic. 
...
We will then cluster cells along the borders - to a resolution of 90m. 
Hence the 3arc data set will remain native resolution, and the 1 arc will have 1 in 3 cells from a window chosen, and 1/3 arc have one in nine cells be selected for calculations of distance between occupied and unoccupied patches. 
...
patchesDistances <- function(x){
  ...
  for (i in 1:dim(patches)[3]){
    ...
    if(grepl('1-3arc-', x)){ # remove 10x10m 'patches' or flecks from the finest res data set.
