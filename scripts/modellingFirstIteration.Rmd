...
OrderRandomPtsIDS <- function(trails, points){
  ...
  duplicate_pts <- raw_pts[lengths( st_intersects(raw_pts)) > 1, ]
  ...
  final_pts <- raw_pts[! raw_pts$UID %in% duplicate_pts$UID, -which(names(raw_pts) %in% c("UID","Pts"))]
  
  return(final_pts)
}

ob <- OrderRandomPtsIDS(trails, thinned)
...
ggplot() +
 # geom_sf(data = PTS) + 
  geom_sf(data = ob, aes(color = Trail)) #+
